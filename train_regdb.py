...
from clustercontrast.trainers import ClusterContrastTrainer_DCL, ClusterContrastTrainer_SVIP
...
from augment import ChannelAdapGray, ChannelRandomErasing, ChannelExchange
...
def get_test_loader(dataset, height, width, batch_size, workers, testset=None, test_transformer=None):
    ...
    if test_transformer is None:
        test_transformer = T.Compose([
            T.Resize((height, width), interpolation=InterpolationMode.BILINEAR),
            T.ToTensor(),
            normalizer
        ])
...
def main_worker_stage2(args, log_s1_name, log_s2_name):
    ...
    optimizer = torch.optim.Adam(params, lr=args.lr, weight_decay=args.weight_decay)
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=args.step_size, gamma=0.1)
    trainer = ClusterContrastTrainer_SVIP(model, model_ema)
    ...
    for epoch in range(args.epochs):
        ...
        memory_ir = ClusterMemory(model.module.num_features, num_cluster_ir, temp=args.temp,
                                  momentum=args.momentum, mode='CMhard', smooth=args.smooth,
                                  num_instances=args.num_instances).cuda()
        memory_rgb = ClusterMemory(model.module.num_features, num_cluster_rgb, temp=args.temp,
                                   momentum=args.momentum, mode='CMhard', smooth=args.smooth,
                                   num_instances=args.num_instances).cuda()
        ...
        if num_cluster_rgb >= num_cluster_ir:
            ...
            for r, i in zip(row_ind, col_ind):
                if i < similarity.shape[1]:
                    r2i[r] = i
                    i2r[i] = r
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="SVIP")
    parser.add_argument('-d', '--dataset', type=str, default='dukemtmcreid', choices=datasets.names())
